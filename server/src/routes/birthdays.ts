...
export const registerBirthDays = (server: Server) => {
  ...
  server.get("/api/birthdays/:code/next", res("json"), (ctx) =>
    withConnection(async (client) => {
      const { code } = ctx.params;
  ...
  server.post("/api/birthdays", req("json"), res("json"), (ctx) =>
    withConnection(async (client) => {
      const birthday = {
        code: ctx.body.code,
      ...
      if (
        ...
        ctx.res.status = 422;
        ctx.res.body = {
          message: "Invalid code, must be a string between 6 and 32 characters",
        };
        return;
      }
      ...
      if (typeof birthday.name !== "string" || birthday.name.length >= 32) {
        ctx.res.status = 422;
        ctx.res.body = {
          message: "Invalid name, must be a string smaller than 32 characters",
        };
        return;
      }
